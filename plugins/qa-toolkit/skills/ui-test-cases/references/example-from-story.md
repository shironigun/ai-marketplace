...
A concrete model showing the full workflow: read the story, plan coverage, write cases. The product domain here (a CRM's Leads module) is only an example — the format and voice are what to copy.
...
```
Preconditions: Environment: QA | Logged in as a user with Admin role
...
```
Preconditions: Environment: QA | A source mapping exists for source "Website" -> Salesperson: John Smith
...
```
Preconditions: Environment: QA | The lead source "Walk-In" has no salesperson mapping configured
...
```
Preconditions: Environment: QA | A source mapping exists for source "Website" -> Salesperson: Jane Doe; Jane Doe's account has been deactivated
...
| 11 | Navigate to the admin notification area | The admin notification inbox opens |
...
```
Preconditions: Environment: QA | Logged in as a user with Salesperson role (non-Admin)
...
- **AC #4 edge**: The inactive-salesperson case needed 12 steps because two things must be verified: unassignment AND notification. They cannot be collapsed.
...
- **Preconditions carry the data state**: the steps do not set up Jane Doe's deactivation — that goes in Preconditions.
